...
// Open Food Facts search functionality
document.getElementById('searchButtonOFF').addEventListener('click', performSearchOFF);
document.getElementById('searchInputOFF').addEventListener('keypress', function(e) {
    if (e.key === 'Enter') {
        performSearchOFF();
    }
});

function performSearchOFF(page = 1) {
    const query = document.getElementById('searchInputOFF').value.trim();
    if (!query) return;

    const searchResults = document.getElementById('searchResultsOFF');
    ...
    fetch(`/api/search/off?q=${encodeURIComponent(query)}&page=${page}`)
        ...
        .then(data => {
            ...
            data.results.forEach(food => {
                ...
                button.innerHTML = `
                    ...
                    ${food.imageUrl ? `<img src="${food.imageUrl}" class="img-thumbnail mt-2" style="max-height: 50px;">` : ''}
                `;
                button.addEventListener('click', () => {
                    updateSankey(food.fdcId);
                    searchResults.style.display = 'none';
                    document.getElementById('searchInputOFF').value = '';
                });
            ...
            if (data.totalPages > 1) {
                ...
                if (data.currentPage > 1) {
                    ...
                    prevButton.onclick = () => performSearchOFF(data.currentPage - 1);
                ...
                if (data.currentPage < data.totalPages) {
                    ...
                    nextButton.onclick = () => performSearchOFF(data.currentPage + 1);
